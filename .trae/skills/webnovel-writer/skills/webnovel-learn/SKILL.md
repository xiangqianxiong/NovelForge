---
name: "webnovel-learn"
description: "从当前会话或用户输入中提取可复用写作模式，写入项目目录的记忆文件。先检查是否存在已确认书名的项目目录（如"逆天改命/"）。"
---
...
从当前会话或用户输入中提取可复用的写作模式，写入项目目录下的长期记忆文件。
...
2. 找到项目目录后，将学习内容写入该目录的 `.webnovel/` 或 `设定集/`
3. 找不到项目目录时，提示"尚未初始化小说项目，请先运行 `/webnovel-init`

**规则：永远在项目目录内存储学习成果。找不到项目目录就不写入。**
